...
async def esegui_analisi_mcp(
    ...
    def _ha_evidenza_corroborante(verdetto: str, risultati: list) -> bool:
        ...
        non solo che il campo esista nel JSON (bug precedente: 'pps_aggregati'
        compare in OGNI output di get_rate_statistics, quindi 'corroborava'
        sempre DOS_VOLUMETRIC anche con PPS=4).
        """
        s = config.Soglie
    ...
    try:
            ...
            async with ClientSession(read_stream, write_stream) as session:
                ...
                if turno >= max_turns and stato_investigazione == "ESPLORAZIONE":
                    ...
                    try:
                        ...
                        # NUOVO: forza structured output invece di free-text
        ...
        # Log del traceback completo di ogni sub-exception per capire se e' un
        # sintomo innocuo (chiusura del context manager dopo un timeout gestito
        # a monte) o una vera perdita di dati durante lo shutdown. Va SOLO in
        # log_only (non log_print) per non sporcare la console dei benchmark,
        # ma resta disponibile nel LOG_*.txt salvato su disco.
            ...
            try:
                ...
                if json_match:
                    ...
                    if v_estratto in prompts.VERDETTI_AMMESSI and mot_estratta:
                        ...
                        # Si applica SOLO se lo Stage 1 era un giudizio esplicito dell'LLM (non un fallback euristico):
                        # se il thought era malformato/assente, non c'è nulla da "proteggere" — lo score è solo un numero.
                        if (
                            verdetto_vincolante_str in ("WEB_ATTACK_EXPLOIT", "DOS_VOLUMETRIC", "SCAN_BRUTEFORCE", "BEACONING_C2")
                            ...
                            # non blocchiamo, ma richiediamo comunque che la motivazione lo segnali esplicitamente
