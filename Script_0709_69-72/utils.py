...
# 1 client
def estrai_porte_realmente_presenti(
    risultati_tool_raccolti: List[Dict[str, Any]], 
    engine: Any = None, 
    ip_target: str = None
) -> Set[int]:
    """
    Estrae ricorsivamente tutte le porte di rete (destination/source/list) 
    presenti nei risultati dei tool raccolti durante l'investigazione.
    """
    porte_trovate: Set[int] = set()

    # 1. Estrazione tramite l'engine (se fornito e disponibile)
    if engine and hasattr(engine, "estrai_flag_deterministici") and ip_target:
        try:
            flags_temp = engine.estrai_flag_deterministici(risultati_tool_raccolti, ip_target)
            for p in flags_temp.get("PORTE_DESTINAZIONE_UNISTICHE", []):
                if isinstance(p, (int, str)) and str(p).isdigit():
                    porte_trovate.add(int(p))
        except Exception:
            pass

    # Chiavi tipiche nei payload dei log/tool che indicano una porta di rete
    KEYS_PORTE = {
        "dst_port", "src_port", "port", "destination_port", 
        "source_port", "target_port", "service_port", "ports"
    }

    def _ricerca_ricorsiva(obj: Any):
        if isinstance(obj, dict):
            for k, v in obj.items():
                # Controllo se la chiave è una combinazione nota per le porte
                if k.lower() in KEYS_PORTE:
                    if isinstance(v, (int, float)):
                        if 1 <= int(v) <= 65535:
                            porte_trovate.add(int(v))
                        ...
                        for elem in v:
                            if isinstance(elem, (int, float)):
                                if 1 <= int(elem) <= 65535:
                                    porte_trovate.add(int(elem))
                            elif isinstance(elem, str) and elem.isdigit():
                                if 1 <= int(elem) <= 65535:
                                    porte_trovate.add(int(elem))
                else:
                    _ricerca_ricorsiva(v)
        elif isinstance(obj, list):
            for item in obj:
                _ricerca_ricorsiva(item)

    # 2. Parsing ed estrazione da tutti i risultati dei tool
    for res_item in risultati_tool_raccolti:
        res_raw = res_item.get("result", "")
        
        # Se il risultato è una stringa JSON, fa il parse prima di analizzarlo
        if isinstance(res_raw, str):
            res_raw = res_raw.strip()
            if res_raw.startswith(("{", "[")):
                try:
                    res_obj = json.loads(res_raw)
                    _ricerca_ricorsiva(res_obj)
                    continue
                except json.JSONDecodeError:
                    pass
        
        # Analisi se è già un dizionario/lista Python o testo generico
        _ricerca_ricorsiva(res_raw)

    return porte_trovate
# 1 client
def tronca_json_sicuro(
    testo_input: str, 
    max_chars: int = config.Soglie.TRONCAMENTO_TOOL_RAW_MAX
) -> str:
    """Tronca una stringa contenente JSON in modo da preservare la validita' sintattica

    o taglia al valore/riga completo più vicino.
    """
    if len(testo_input) <= max_chars:
        return testo_input

    # Tentativo 1: Parsing e strutturazione JSON pulita
    try:
        data = json.loads(testo_input)
        if isinstance(data, list):
            # Riduci il numero di elementi finché la rappresentazione rientra nel limite
            lista_ridotta = list(data)
            while lista_ridotta and len(json.dumps(lista_ridotta)) > max_chars:
                lista_ridotta.pop()

            if lista_ridotta:
                res = json.dumps(lista_ridotta, indent=2)
                return f"{res}\n\n[... {len(data) - len(lista_ridotta)} ELEMENTI TRONCATI PER CONTESTO ...]"

        elif isinstance(data, dict):
            # Se è un dizionario (es. con chiave 'flows' o 'data')
            for key in ["flows", "data", "results"]:
                if key in data and isinstance(data[key], list):
                    while (
                        data[key] and len(json.dumps(data)) > max_chars
                    ):
                        data[key].pop()
                    res = json.dumps(data, indent=2)
                    return f"{res}\n\n[... ALCUNI ELEMENTI IN '{key}' TRONCATI PER CONTESTO ...]"
    except Exception:
        pass  # Se non è un JSON valido o il parsing fallisce, usa il fallback testuale

    # Fallback 2: Taglio basato sulle righe (evita di spezzare una riga/chiave a metà)
    testo_tagliato = testo_input[:max_chars]
    ultimo_newline = testo_tagliato.rfind("\n")

    if ultimo_newline > 0:
        testo_tagliato = testo_tagliato[:ultimo_newline]

    return f"{testo_tagliato}\n\n[... OUTPUT TRUNCATED FOR CONTEXT OPTIMIZATION ...]"
...
def estrai_verdetto_pulito(*args) -> str:
    ...
    for arg in reversed(args):
        # Supporto per oggetti ChatCompletion di OpenAI/Groq
    ...
    # 1. Ricerca diretta per linea
    lines = testo.split('\n')
    for line in reversed(lines):
        line_clean = line.strip().replace("*", "").replace("#", "").replace("`", "")
        if any(p in line_clean.upper() for p in ["VERDETTO:", "VERDETTO FINALE:", "CLASSIFICAZIONE:"]):
            parti = line_clean.split(":", 1)
            if len(parti) > 1:
                v_cand = parti[1].strip().upper()
                
                if hasattr(config, "VERDETTI_AMMESSI") and v_cand in config.VERDETTI_AMMESSI:
                    return v_cand
                
                if "BENIGN" in v_cand: return "BENIGN"
                if any(x in v_cand for x in ["BEACON", "C2"]): return "BEACONING_C2"
                if any(x in v_cand for x in ["BRUTE", "SCAN", "PORT"]): return "SCAN_BRUTEFORCE"
                if "DOS" in v_cand: return "DOS_VOLUMETRIC"
                if any(x in v_cand for x in ["WEB", "EXPLOIT", "SQL", "XSS"]): return "WEB_ATTACK_EXPLOIT"

    # 2. Fallback su Regex globale (Cerca la parola chiave ovunque nel testo)
    pattern = r"(?:VERDETTO|CLASSIFICAZIONE)\s*[:=]?\s*[`'\"]*([A-Z0-9\-_]+)[`'\"]*"
    matches = re.findall(pattern, testo, re.IGNORECASE)
    if matches:
        for match in reversed(matches):
            v_cand = match.strip().upper()
            if hasattr(config, "VERDETTI_AMMESSI") and v_cand in config.VERDETTI_AMMESSI:
                return v_cand

    # 3. Last-resort fallback: Cerca se una delle categorie ammesse compare isolata
