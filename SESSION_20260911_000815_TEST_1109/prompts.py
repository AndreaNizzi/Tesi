...
2. ATTACCHI DOS APPLICATIVI L7 / FLOOD VOLUMETRICO (DOS_VOLUMETRIC):
   - Gli attacchi DoS a livello applicativo (Hulk, GoldenEye, Slowloris, SlowHTTPTest) o di rete L4 generano saturazione di risorse.
   - OVERRIDE TASSATIVO DOS_VOLUMETRIC: Assegna TASSATIVAMENTE 'DOS_VOLUMETRIC' se si verifica ALMENO UNA delle seguenti condizioni:
     a) 'get_rate_statistics' segnala 'ANOMALIA_RILEVATA' o 'CRITICO' per flood/burst/rate elevato;
     b) 'get_flow_features' rileva flussi con impronta/suspect Slowloris, Hulk o HTTP DoS ASSOCIATI a volumi sostenuti o allarmi di rate;
     c) Vi è un accumulo MASSIVO di flussi/richieste HTTP/HTTPS (>= 1.000 richieste totali, RPS > 10, o flussi web/connessioni aggregate >= 500);
     d) Il rapporto tra porte sorgente effimere distinte e flussi totali supera il 90% in una finestra con densità di flussi elevata (> 500 flussi), anche con PPS aggregati bassi.
   ...
   - È SEVERAMENTE VIETATO classificare come 'BENIGN' o 'WEB_ATTACK_EXPLOIT' un attacco segnalato come critico/flood da 'get_rate_statistics' solo perché 'compute_verdict_scores' restituisce 0.0 o il PPS medio è basso.
...
Se due regole sembrano applicabili contemporaneamente, applica tassativamente
la GERARCHIA DI DISAMBIGUAZIONE:
...
1. PRIORITÀ 1 - WEB ATTACK / EXPLOIT / WEB BRUTE FORCE [EARLY STOP MANDATORIO]:
   ...
   - OVERRIDE SLOWLORIS / DoS APPLICATIVO: Se un attacco Web invia un numero 
     ...
     Se invece i flussi superano i 150-200 o mostrano un'alta densità di porte effimere (>85%), applica la regola DOS_VOLUMETRIC (Priorità 3).
...
3. PRIORITÀ 3 - DOS VOLUMETRICO / HTTP FLOOD / DNS FLOOD / DoS L7:
   - Classifica come DOS_VOLUMETRIC se il traffico Web genera un flood massivo (RPS > 10, PPS aggregati elevati) OPPURE se presenta il pattern DoS L7 ad alta densità: flussi >= 150-200 con >85% di porte sorgente effimere uniche create in tempi brevi (es. Hulk, GoldenEye, HTTP Flood).
   - Classifica ugualmente come DOS_VOLUMETRIC se si osserva un DNS Flood / Amplification DoS con prevalenza di traffico sulla porta 53 (> 80% dei flussi totali e burst PPS >= 300.0 o flussi >= 20.000).
