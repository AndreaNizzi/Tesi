...
def query_by_rate(
    ...
    if metric not in ["packet_rate", "byte_rate"]:
        return (
            "[ERRORE TOOL]: La metrica deve essere 'packet_rate' o 'byte_rate'."
        )
...
def _calcola_scores_da_evidenze(
    ...
    def _eval_scan_and_bruteforce(conn_attempts: dict, port_ss: dict, s) -> tuple[float, list[str]]:
        note = []
        ...
        # Copre il buco tra "scan largo" (>=15 porte) e "bruteforce concentrato" (>=25
        # tentativi su una porta): poche porte (4-14) ma eterogenee per categoria di
        # servizio (web + gestione + porta non standard) con volumi comparabili tra
        # loro. E' il pattern tipico di un host che tocca 80/443/21/22/porta-custom
        # dello stesso target con 20-45 flussi ciascuna: ricognizione di servizi,
        # non un attacco applicativo mirato a un singolo endpoint.
        ...
        if scan_score < 0.85 and sweep_porte_min <= porte_uniche_total < getattr(s, "SCAN_PORTE_MIN", 15) and distribuzione_top:
            ...
            # Le porte infra LAN (DNS/Kerberos/LDAP) non contano come "categoria di servizio
            # attaccata": altrimenti ogni host normale con traffico DNS sembrerebbe uno sweep.
            categorie_rilevanti = categorie_toccate - {"INFRA_LAN"}

            if len(categorie_rilevanti) >= 2 and concentrazione <= sweep_conc_max:
                scan_score = max(scan_score, 0.85)
                note.append(
                    f"Rilevato sweep multi-servizio a basso volume su {porte_uniche_total} porte "
                    f"eterogenee ({', '.join(sorted(categorie_rilevanti))}), nessuna porta dominante "
                    f"(concentrazione max {concentrazione:.0%})."
                )

        return scan_score, note

    def _eval_dos_scores(mk: dict, l7_ss: dict, flussi_p53: int, s) -> tuple[float, list[str]]:
        note = []
        ...
        if is_dns_traffic:
            ...
            volume_elevato = flussi_web >= 1000 or max_tentativi_ip >= 1000 or effective_web_rps >= 10.0
            concentrazione_ok = concentrazione_web >= concentrazione_dos_min

            if volume_elevato and concentrazione_ok:
                dos_l7_score = 0.95
                note.append(
                    f"Elevata saturazione HTTP concentrata ({flussi_web} flussi su "
                    f"{destinazioni_web} destinazioni, ~{concentrazione_web:.0f} flussi/host): identificato DoS L7."
                )
            elif volume_elevato and not concentrazione_ok:
                # Volume alto ma spalmato su molte destinazioni: browsing intenso, non DoS.
                note.append(
                    f"Volume HTTP elevato ({flussi_web} flussi) ma su {destinazioni_web} destinazioni "
                    f"distinte (~{concentrazione_web:.0f} flussi/host): compatibile con browsing "
                    "intenso, NON con un flood concentrato. Score DoS L7 non assegnato."
                )
        ...
        final_dos = max(dos_pps_score, dos_l7_score, dos_slow_score)
    ...
    def _eval_beacon_score(beacon: dict, dos_score: float, mk: dict) -> tuple[float, list[str]]:
        ...
        if candidati_esterni:
            ...
            # Verifica se ci sono connessioni a porte tipiche C2/Proxy (8080, 8443, 444, 1080)
            ...
            # Riconoscimento C2 con Jittering (CV elevato fino a 1.50) se ci sono ripetizioni coerenti su IP esterno
    ...
    is_volumetric_dos_l7 = flussi_web >= 1000 or max_tentativi_ip >= 1000
    ...
    if anomalie_l7 > 0 or early_stop_exploit:
        ...
        if login_endpoint_targeted:
            ...
            # FIX: senza endpoint di login mirato e senza anomalie L7 reali, il
            # solo conteggio di richieste e' un'evidenza debole. Score ridotto per
            # non pareggiare artificialmente con segnali C2/Scan gia' corroborati
            # da evidenze indipendenti (es. CONFIRMED_BEACONING_C2, sweep multi-porta).
            web_attack_score = 0.40
            note_logiche.append(
                f"Sospetto Web Brute Force a basso volume ({max_tentativi_ip} req/IP) NON "
                "corroborato da endpoint di login o anomalie L7 reali: score ridotto "
                "(possibile falso positivo dell'euristica di conteggio)."
            )

    # 2. Calcolo Altri Score
    scan_score, note_scan = _eval_scan_and_bruteforce(conn_attempts, port_ss, s)
    note_logiche.extend(note_scan)

    dos_score, note_dos = _eval_dos_scores(mk, l7_ss, flussi_p53, s)
    ...
    # Determinazione Suggerimento per l'LLM (Tie-Breaking Eristico)
    ...
    if max_val >= 0.5:
        ...
        if len(vincitori) == 1:
            ...
            # PAREGGIO REALE tra due o più categorie: non decidere per l'LLM.
            # Segnala il conflitto esplicitamente invece di scegliere in base
            # a un ordine statico arbitrario (bug diagnosticato: penalizzava
            # sistematicamente BEACONING_C2 a favore di WEB_ATTACK_EXPLOIT).
            conflitto_a_pari_merito = vincitori
            # Scegliamo comunque un valore di default per compatibilità con
            # i campi a valle, ma il flag conflitto_a_pari_merito segnala
            # che questo valore NON va trattato come deterministico.
            priorita_solo_come_default = ["DOS_VOLUMETRIC", "WEB_ATTACK_EXPLOIT", "SCAN_BRUTEFORCE", "BEACONING_C2"]
            vincitore_assoluto = next((kat for kat in priorita_solo_come_default if kat in vincitori), "BENIGN")
            note_logiche.append(
                f"CONFLITTO A PARI MERITO tra {vincitori} (score {max_val}). "
                "Il tool NON è in grado di determinare deterministicamente quale prevale: "
                "verifica le evidenze grezze per sciogliere il pareggio."
            )
    else:
    ...
    return {
        ...
        "conflitto_a_pari_merito": conflitto_a_pari_merito,  # lista vuota se non c'è pareggio
...
def _detect_beaconing_raw(
    ...
    for riga in righe:
        ...
        anomaly_score, tag_list = _compute_anomaly_score(
            cv, totale_connessioni, is_whitelisted, dst_ip_val, dst_port
        )
...
def _get_host_port_distribution_raw(
    ...
    query = text("""
        ...
        FROM ndpi_flows
        WHERE src_ip = :ip   -- SOLO OUTBOUND: ip_target come iniziatore reale
          AND timestamp_start BETWEEN :start_time AND :end_time
        GROUP BY dst_port
    ...
    # Rilevamento Porte Probe (filtro a due livelli: flussi ridotti E pacchetti medi bassi)
    # Evita che frammenti di flood ad alta densità vengano contati come scan probe
    porte_probe = [
